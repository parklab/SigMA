...
calc_sn_sp_matching <- function(df, signame1, signame2, matching){
  total1 <- dim(df$truth == signame1)[[1]]
  total2 <- dim(df$truth == signame2)[[1]]
  tp1 <- sum(df$truth == signame1 &
                df[,matching] == signame1)
  tp2 <- sum(df$truth == signame2 &
                df[, matching] == signame2)
  fp1 <- sum(merged$truth == signame2 &
                df[, matching] == signame1)
  fp2 <- sum(merged$truth == signame1 &
                df[, matching] == signame2)
...
sn_sp_vs_nsnv <- function(df, signame1, signame2, snv_ranges, matching) 
  sp_vec1 <- rep(0, length(snv_ranges))
  sp_vec1 <- rep(0, length(snv_ranges))
  sn_vec2 <- rep(0, length(snv_ranges))
  sn_vec2 <- rep(0, length(snv_ranges))

  for(isnv in 1:(length(snv_ranges) - 1)){
    df_this <- df[df$total_snvs >= nsnv_ranges[[isnv]] &
                  df$total_snvs < nsnv_ranges[[isnv + 1]],]
  ...
  df1 <- data.frame(sp = sp_vec1, 
                    ...
                    nsnv_high = nsnv_high[2:length(snv_ranges)],
                    truth = signame1,
                    method = matching)
           ...
           cutoff_l = 0.25)
{
  ...
  if(dependence == "total_snvs"){
    df_l <- sn_sp_vs_nsnv(merged, signame1, signame2, snv_ranges, 'sig_max_l')
    df_c <- sn_sp_vs_nsnv(merged, signame1, signame2, snv_ranges, 'sig_max_c')   
    df <- rbind(df_l, df_c) 
